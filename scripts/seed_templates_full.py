"""Seed templates: DB insert + S3 notebook upload. Run inside master container."""
import json
import uuid
import boto3
import pymysql
...
DB_CONFIG = {"host": "ainotebook-mysql", "port": 3306, "user": "root",
             "password": "ainotebook_dev_password", "database": "ainotebook"}
S3_CONFIG = {"endpoint_url": "http://ainotebook-minio:9000",
...
TEMPLATES = [
    {
        ...
        "sort_order": 0,
        "notebook": mk([code("# Start coding here\n")]),
    },
    {
        "name": "Python Basics",
        "description": "Learn Python fundamentals: variables, data types, loops, functions, and basic I/O.",
        "category": "Learning",
        "difficulty_level": "beginner",
    ...
    },
    {
        ...
        "description": "Fetch and process data from REST APIs using the requests library.",
        "category": "Web & APIs",
    ...
    },
    {
        ...
        "description": "Build your first ML model with scikit-learn: data prep, train/test split, training, and evaluation.",
...
cursor = conn.cursor()
cursor.execute("SELECT COUNT(*) FROM notebook_templates")
count = cursor.fetchone()[0]
if count > 0:
    print(f"Templates already exist ({count} found). Skipping.")
    cursor.close()
    conn.close()
    exit(0)
...
for tmpl in TEMPLATES:
    tid = str(uuid.uuid4())
    spath = f"templates/{tid}/notebook.ipynb"
    now = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")

    cursor.execute(
        "INSERT INTO notebook_templates (id,name,description,category,storage_path,difficulty_level,estimated_minutes,tags,is_public,created_by,sort_order,created_at,updated_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
        (tid, tmpl["name"], tmpl["description"], tmpl["category"], spath,
         tmpl["difficulty_level"], tmpl["estimated_minutes"],
         json.dumps(tmpl["tags"]), True, ADMIN_USER_ID, tmpl["sort_order"], now, now)
    )

    s3.put_object(
        Bucket=S3_BUCKET, Key=spath,
        Body=json.dumps(tmpl["notebook"], indent=2).encode("utf-8"),
        ContentType="application/json",
    )
    print(f"  [+] {tmpl['name']} ({len(tmpl['notebook']['cells'])} cells)")

conn.commit()
cursor.close()
conn.close()
print(f"\nDone! Inserted {len(TEMPLATES)} templates.")
